...
wget -O ~/lotus_testnet_genesis_docker.sh https://github.com/lotuscommunity/lotus-testnet/raw/main/lotus_testnet_genesis_docker.sh \
&& chmod +x ~/lotus_testnet_genesis_docker.sh && ./lotus_testnet_genesis_docker.sh
```
...
3. Running the Lotus Validator(in the Docker container)

Attach Docker containers, start a `tmux` session and run the `lotus node` command.
...
[//]: # ()
[//]: # (2. Running the Lotus Validator&#40;in the Docker container&#41;)
...
## Step 4: Running the Lotus VFN and Validators(outside the container)
...
```bash
apt update && apt install nano && apt install wget
wget -O ~/lotus_testnet_setup.sh https://github.com/lotuscommunity/lotus-testnet/raw/main/lotus_testnet_setup.sh \
&& chmod +x ~/lotus_testnet_setup.sh && ./lotus_testnet_setup.sh
```
...
https://github.com/lotuscommunity/lotus/blob/96188bdf4b67ebc8e6cc189ef7ed24484076920b/util/fixtures/mnemonic/alice.mnem
...
If you're in the genesis-post validator, enter the mnemonic for your own account, modify seed_addrs in `~/.lotus/validator.yaml` with VFN's.
